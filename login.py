...
class login:
    def __init__(self, root):
        ...
        login_frame.place(x=550, y=220, width=550, height=420)
        ...
        login_btn.place(x=50, y=205, anchor=NW) #y=200
        ...
        reset_btn.place(x=275, y=205, anchor=NW)

        # line seperator
        lineSeperator_frame = Frame(
            login_frame, bd=0, relief=GROOVE, bg="white", highlightthickness=5
        )
        lineSeperator_frame.place(x=-7, y=270, width=550, height=143) 
    ...
    def login_details(self):
        if (
            ...
            try:  # Now we will connect with SQL
                ...
                # ===================cursor 0=============================     To run this below connection - pip install mysql-connector-python
                # to check the credentials
                my_cursor = conn.cursor() # To store the values given by the user
                temp_email = "'"+temp_email+"'"
                sql = 'select password from {} where email={}'.format(str(temp_table_name),str(temp_email))
                print(sql)
                ...
                conn.commit() # Commit says that changes made in this session are permanent and will be further shown for other sessions
                # self.fetch_data()
